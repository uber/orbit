...
transformed parameters {
  ...
  vector[NUM_OF_OBS + 1] b; // local slope
  vector[NUM_OF_OBS + SEASONALITY] s;
  vector[NUM_OF_OBS + 1] gt_sum; // sum of global trend
  ...
  for (t in 1:NUM_OF_OBS) {
    ...
    err[t] = RESPONSE[t] - gt_sum[t] - (l[t] + s[t] + DAMPED_FACTOR * b[t]);
