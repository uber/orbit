...
parameters {
  ...
  vector<lower=0,upper=0.01>[NUM_OF_REG] b_sm;
  ...
  for (t in 2:NUM_OF_OBS) {
    ...
    if (NUM_OF_REG >= 1) {
      for (i in 1:NUM_OF_REG) {
        ...
        delta = REG_MAT[t, i] - REG_MAT[t-1,i];
        if (delta > 1e-5) {
          b[t+1,i] = b[t,i] + b_sm[i] * err[t]/delta;
        } else {
          b[t+1,i] = b[t,i];
        }
      }
