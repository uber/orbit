...
from ..constants import tkr as constants
...
class BaseTKR(BaseModel):
    ...
    def __init__(self,
        ...
        self._regressor_col = regressor_col
        self._regressor_knot_loc = list()
        self._regressor_knot_scale = list()
        # this will be a subset of _regressor_col
        self._seasonal_regressor_col = list()
    ...
    def _set_default_base_args(self):
        ...
        elif len(self._seasonality_fs_order) != len(self._seasonality):
            raise IllegalArgument('length of seasonality and fs_order not matching')

        # TODO: how to solve this along with self._seasonality_fs_order?
        # self._seasonality = sorted(list(set(self._seasonality)))
        ...
        _validate([self.regressor_knot_loc, self.regressor_knot_scale], self._num_of_regressors)

        if self.regressor_knot_loc is None:
            self._regressor_knot_loc = [DEFAULT_COEFFICIENTS_LOC] * self._num_of_regressors

        if self.regressor_knot_scale is None:
            self._regressor_knot_scale = [DEFAULT_COEFFICIENTS_SCALE] * self._num_of_regressors
    ...
    def _set_seasonality_attributes(self):
        ...
        if len(self._seasonality) > 0:
            ...
            flatten_seasonal_regressor_col = []
            for idx, s in enumerate(self._seasonality):
                fs_cols = []
                order = self._seasonality_fs_order[idx]
                for i in range(1, order + 1):
                    fs_cols.append('layer{}_fs_cos{}'.format(idx, i))
                    fs_cols.append('layer{}_fs_sin{}'.format(idx, i))
                self._seasonal_regressor_col.append(fs_cols)
                flatten_seasonal_regressor_col += fs_cols

            # update all regressors related attributes
            self._num_of_regressors += len(flatten_seasonal_regressor_col)
            self._regressor_col = flatten_seasonal_regressor_col + self._regressor_col
            self._regressor_knot_loc = [0.0] * len(flatten_seasonal_regressor_col) + self._regressor_knot_loc
            self._regressor_knot_scale = [1.0] * len(flatten_seasonal_regressor_col) + self._regressor_knot_scale
    ...
    def _set_regressor_matrix(self, df):
        ...
        if self._num_of_regressors > 0:
            self._regressor_matrix = df[self._regressor_col].values
    ...
    def _predict(self, posterior_estimates, df, include_error=False, decompose=False):
        ...
        if decompose:
            ...
            if self._num_of_regressors > 0:
                ...
                if len(self._seasonal_regressor_col) > 0:
                    pos = 0
                    for idx, cols in enumerate(self._seasonal_regressor_col):
                        seasonal_regressor = df[cols].values
                        sub_coef = coef[..., pos:pos+len(cols), :]
                        regression = np.sum(sub_coef * seasonal_regressor.transpose(1, 0), axis=-2)
                        decomp_dict['seasonality_{}'.format(idx)] = regression
                        pos += len(cols)
                        total_regression += regression
                # get rest of regression components
                flatten_list = [item for sublist in self._seasonal_regressor_col for item in sublist]
                regressor_cols = list(set(self._regressor_col) - set(flatten_list))
                if len(regressor_cols) > 0:
                    regressor_matrix = df[regressor_cols].values
                    sub_coef = coef[pos:pos + len(regressor_cols)]
                    regression = np.sum(sub_coef * regressor_matrix.transpose(1, 0), axis=-2)
                    decomp_dict['regression'] = regression
                    total_regression += regression

            decomp_dict['trend'] = trend
            pred_array = trend + total_regression
            decomp_dict['prediction'] = pred_array

            return decomp_dict
        else:
            # without decomposition, we don't need to distinguish seasonality from total regression
            regression = np.zeros((output_len, 0), dtype=np.double)
            if self._num_of_regressors > 0:
                regressor_matrix = df[self._regressor_col].values
                regression = np.sum(np.matmul(coef_knot, kernel_coefficients.transpose(1, 0)) * \
                                    regressor_matrix.transpose(1, 0), axis=-2)
    ...
    def get_regression_coefs(self, aggregate_method, include_ci=False):
        ...
        if self.num_of_regressors + self._num_of_positive_regressors == 0:
            return reg_df
