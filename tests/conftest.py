...
def make_daily_data(request):
    ...
    if seasonality is None:
        ...
        fs = make_seasonality(
            n_obs, seasonality=365.25, method="fourier", order=2, seed=seed
        )
    elif seasonality == "dual":
        fs1 = make_seasonality(
            n_obs, seasonality=365.25, method="fourier", order=3, seed=seed
        )
        fs2 = make_seasonality(
            n_obs, seasonality=7, method="fourier", order=2, seed=seed
        )
        fs = fs1 + fs2
    ...
    if with_coef:
        ...
        df = pd.DataFrame(
            np.concatenate([response, x], axis=1), columns=["response"] + list("abc")
        )
    else:
        response = rw + fs
        coef = None
...
def make_weekly_data():
    ...
    df = pd.DataFrame(
        np.concatenate([(rw + fs + y).reshape(-1, 1), x], axis=1),
        columns=["response"] + list("abcdef"),
    )
